...
class UNION_LSTM1D(Module_Mdl):	#	f(ax0+bx1+cx2+d)
	...
	def cree_ix(self):
		...
		self.elements = {
			...
			'f' : DOT1D_1(X=[X+Y+Y], Y=[Y], params={'activ':logistique}, do=self.do,dc=self.dc).cree_ix(),
			# i = logistique(sI = Ix@x + Ih@h[-1] + Ic@c[-1] + Ib)
			'i' : DOT1D_1(X=[X+Y+Y], Y=[Y], params={'activ':logistique}, do=self.do,dc=self.dc).cree_ix(),
			#u =       tanh(sU = Ux@x + Uh@h[-1] +          + Ub)
			'u' : DOT1D_1(X=[X+Y], Y=[Y], params={'activ':_tanh}, do=self.do,dc=self.dc).cree_ix(),
			#c = f*c[-1] + i*u
			...
			'o' : DOT1D_1(X=[X+Y+Y], Y=[Y], params={'activ':logistique}, do=self.do,dc=self.dc).cree_ix(),
			#h = o * ch
