...
__global__
static void d_kerd__somme4(
	uint x0_t, uint X0, float * x0, float * dx0,
	uint x1_t, uint X1, float * x1, float * dx1,
	uint x2_t, uint X2, float * x2, float * dx2,
	uint x3_t, uint X3, float * x3, float * dx3,
	//
	uint    Y,
	float * y, float * dy,
	//
	uint * ts__d, uint mega_t)
{
	...
	if (_y < Y && _t < GRAND_T) {
		...
		atomicAdd(&dx1[tx1*X1 + _y], 1 * __dy);
		atomicAdd(&dx2[tx2*X2 + _y], 1 * __dy);
		atomicAdd(&dx3[tx3*X3 + _y], 1 * __dy);
	}
};
...
void somme4__df(Inst_t * inst, float ** x__d, float ** dx__d, uint * ts__d, uint mega_t) {
	...
	if (xs_existants == 4) {
		...
		//
		d_kerd__somme4<<<dim3(KERD(Y,16), KERD(GRAND_T,16)), dim3(16,16)>>>(
			...
			inst->x_t[_i3], inst->x_Y[_i3], x__d[_i3], dx__d[_i3],
			//
			inst->Y,
