	...
	uint * ts__d, uint mega_t)
{
	...
	if (_y < Y && _t < GRAND_T) {
		...
		float __x0 = x0[tx0*X0 + _y];
		float __x1 = x1[tx1*X1 + _y];
		float __x2 = x2[tx2*X2 + _y];
		float __dy = dy[ty*Y + _y];
		atomicAdd(&dx0[tx0*X0 + _y], __dy *   1  * __x1 * __x2);
		atomicAdd(&dx1[tx1*X1 + _y], __dy * __x0 *   1  * __x2);
		atomicAdd(&dx2[tx2*X2 + _y], __dy * __x0 * __x1 *   1 );
	}
};
...
void mul3__df(Inst_t * inst, float ** x__d, float ** dx__d, uint * ts__d, uint mega_t) {
	...
	if (xs_existants == 3) {
		...
		//
		d_kerd__mul3<<<dim3(KERD(Y,16), KERD(GRAND_T,16)), dim3(16,16)>>>(
