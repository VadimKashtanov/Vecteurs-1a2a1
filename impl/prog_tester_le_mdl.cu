...
static float ** toutes_les_predictions(Mdl_t * mdl, BTCUSDT_t * btcusdt) {
	...
	FOR(0, _t_, T) {
		...
		//
		uint Y = mdl->inst[mdl->la_sortie]->Y;
		float * y = gpu_vers_cpu<float>(mdl->inst[mdl->la_sortie]->y__d, GRAND_T*MEGA_T*Y);
		FOR(0, mega_t, MEGA_T) {
			uint ty = t_MODE(0, mega_t);
			les_Amplitudes [lp] = y[ty*Y + 0];
			les_predictions[lp] = y[ty*Y + 1];
